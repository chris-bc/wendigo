...
typedef enum {
    WENDIGO_BT_SCAN_CLASSIC_DISCOVERY = 0,
    WENDIGO_BT_SCAN_BLE,
    WENDIGO_BT_SCAN_TYPE_ACTIVE,
    WENDIGO_BT_SCAN_TYPE_COUNT
} wendigo_bt_scan_t;
...
extern app_gap_cb_t **wendigo_selected_bt;
extern uint8_t wendigo_sel_bt_count;
...
esp_err_t wendigo_bt_list_devices(app_gap_cb_t **devices, uint8_t deviceCount, bool hideExpiredPackets);
esp_err_t wendigo_clear_bt();
esp_err_t wendigo_clear_bt_selected();
esp_err_t wendigo_select_bt(uint8_t selIndex);
bool wendigo_bt_isSelected(uint8_t selIndex);
esp_err_t wendigo_bt_disable_scan();
void *wendigo_ble_purge_and_malloc(uint16_t bytes);
esp_err_t wendigo_bt_shrink_devices();
char *purgeStrategyToString(char *strOutput);
esp_err_t purgeBLE(uint16_t minAge, int32_t maxRssi);
esp_err_t purgeBT(uint16_t minAge, int32_t maxRssi);
...
esp_err_t bt_dev_add_components(esp_bd_addr_t bda, char *bdName, uint8_t bdNameLen, uint8_t *eir, uint8_t eirLen,
                        uint32_t cod, int32_t rssi, wendigo_bt_scan_t devScanType);
...
esp_err_t wendigo_bt_discover_selected_services();
