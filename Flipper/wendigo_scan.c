 ...
 *  This function does not alter buffer capacity, but updates buffer and
 * bufferLen to remove the specified number of bytes from the beginning of the
 * buffer. If the specified number of bytes is greater than the current buffer
 * length then bufferLen bytes are removed from the buffer, emptying it.
 */
void consumeBufferBytes(uint16_t bytes) {
    FURI_LOG_T(WENDIGO_TAG, "Start consumeBufferBytes()");
    ...
    bufferLen -= bytes;
    FURI_LOG_T(WENDIGO_TAG, "End consumeBufferBytes()");
}
...
/* Returns the index of the first occurrence of '\n' in `theString`, or
   strlen(theString) if not found. This function requires `theString` to be null
   terminated. If theString is NULL or if strlen(theString) > 0xFFFF (65535,
   MAX_UINT16) the function returns 0xFFFF. If this value is returned and
   theString is not NULL it's a good indication the string is not
   NULL-terminated.
   TODO: This function is a candidate for deletion. */
uint16_t string_contains_newline(char* theString) {
    if(theString == NULL || strlen(theString) > 0xFFFF) {
        return 0xFFFF;
    }
...
bool wendigo_set_device_selected(wendigo_device* device, bool selected) {
    ...
    if(selected && !device->tagged) {
        ...
        if(selected_devices_capacity == selected_devices_count) {
            ...
            wendigo_device** new_devices = realloc(
                selected_devices, sizeof(wendigo_device*) * (selected_devices_capacity + 1));
            if(new_devices == NULL) {
     ...
     * handle any NULLs */
        ...
        uint8_t index =
            (uint8_t)custom_device_index(device, selected_devices, selected_devices_count);
...
bool wendigo_add_bt_device(wendigo_device* dev, wendigo_device* new_device) {
    ...
    if(dev->radio.bluetooth.bdname_len > 0 && dev->radio.bluetooth.bdname != NULL) {
        new_device->radio.bluetooth.bdname =
            malloc(sizeof(char) * (dev->radio.bluetooth.bdname_len + 1));
        if(new_device->radio.bluetooth.bdname == NULL) {
            /* Skip the device's bdname and hope we have memory for it next time it's
       * seen */
    ...
    if(dev->radio.bluetooth.eir_len > 0 && dev->radio.bluetooth.eir != NULL) {
        ...
        if(new_device->radio.bluetooth.eir == NULL) {
            ...
            memcpy(
                new_device->radio.bluetooth.eir,
...
bool wendigo_update_bt_device(wendigo_device* dev, wendigo_device* new_device) {
    ...
    /* How about EIR? */
    if(dev->radio.bluetooth.eir_len > 0 && dev->radio.bluetooth.eir != NULL) {
        uint8_t* new_eir = realloc(
            new_device->radio.bluetooth.eir, sizeof(uint8_t) * dev->radio.bluetooth.eir_len);
        if(new_eir != NULL) {
            memcpy(
                new_eir, dev->radio.bluetooth.eir, sizeof(uint8_t) * dev->radio.bluetooth.eir_len);
 ...
 *  Upon completion, `dev` will contain a pointer to the wendigo_device
 * represented by this MAC, if it is known to Wendigo.
 */
bool wendigo_link_wifi_devices(
    WendigoApp* app,
    wendigo_device* dev,
    uint8_t** macs,
    uint8_t mac_count) {
    ...
    if(dev->scanType == SCAN_WIFI_STA) {
       ...
       well as all stations specified in macs[] */
        ...
        uint8_t count = 0;
        uint16_t real_index = device_index(dev);
        ...
        if(real_index < devices_count) {
            ...
            for(uint8_t i = 0; i < mac_count; ++i) {
                ...
                if(macs[i] != NULL) {
                    ...
                    temp_idx = custom_device_index(
                        &temp_device,
                        (wendigo_device**)real_device->radio.ap.stations,
                        real_device->radio.ap.stations_count);
                    /* Also confirm that temp_device is a known device */
                    temp_idx2 = device_index(&temp_device);
                    if(temp_idx == real_device->radio.ap.stations_count &&
                       temp_idx2 < devices_count) {
                        /* macs[i] is not in real_device->radio.ap.stations and IS in
               devices[]
               => We need to add it to stations[] */
                        ++count;
                    }
                }
            }
        } else {
            /* Count the MACs in macs[] that are in devices[] */
            for(uint8_t i = 0; i < mac_count; ++i) {
        ...
        if(count > 0) {
            ...
            if(real_device != NULL && real_device->radio.ap.stations_count > 0 &&
               real_device->radio.ap.stations != NULL) {
                memcpy(
                    dev->radio.ap.stations,
                    real_device->radio.ap.stations,
                    sizeof(wendigo_device*) * real_device->radio.ap.stations_count);
            ...
            for(uint8_t i = 0; i < mac_count; ++i) {
                if(macs[i] != NULL) {
                    ...
                    memcpy(temp_device.mac, macs[i], MAC_BYTES);
                    if(real_device != NULL) {
                        temp_idx2 = custom_device_index(
                            &temp_device,
                            (wendigo_device**)real_device->radio.ap.stations,
                            real_device->radio.ap.stations_count);
                    }
                    /* Append the current station if it's in devices[] and not in
           * real_device->radio.ap.stations[] */
                    if(temp_idx < devices_count &&
                       (real_device == NULL ||
                        temp_idx2 == real_device->radio.ap.stations_count)) {
...
bool wendigo_add_device(WendigoApp* app, wendigo_device* dev) {
    ...
    /* Adding to devices - Increase capacity by an additional
   * INC_DEVICE_CAPACITY_BY if necessary */
    if(devices == NULL || devices_capacity == devices_count) {
        wendigo_device** new_devices = realloc(
            devices, sizeof(wendigo_device*) * (devices_capacity + INC_DEVICE_CAPACITY_BY));
        if(new_devices == NULL) {
    ...
    */
        ...
        if(dev->radio.ap.stations_count > 0) {
            /* Copy stations[] */
            new_device->radio.ap.stations =
                malloc(sizeof(wendigo_device*) * dev->radio.ap.stations_count);
 ...
 */
void process_and_display_status_attribute(
    WendigoApp* app,
    char* attribute_name,
    char* attribute_value) {
    FURI_LOG_T(WENDIGO_TAG, "Start process_and_display_status_attribute()");
    char* interesting_attributes[] = {
        "BT Classic Devices:", "BT Low Energy Devices:", "WiFi STA Devices:", "WiFi APs:"};
...
uint16_t parseBufferBluetooth(WendigoApp* app) {
   ...
   */
    ...
    memcpy(
        &(dev->radio.bluetooth.bdname_len),
        buffer + WENDIGO_OFFSET_BT_BDNAME_LEN,
        sizeof(uint8_t));
    ...
    memcpy(
        &(dev->radio.bluetooth.bt_services.num_services),
        buffer + WENDIGO_OFFSET_BT_NUM_SERVICES,
        sizeof(uint8_t));
    memcpy(
        &(dev->radio.bluetooth.bt_services.known_services_len),
        buffer + WENDIGO_OFFSET_BT_KNOWN_SERVICES_LEN,
        sizeof(uint8_t));
    uint8_t cod_len;
    ...
    if(dev->radio.bluetooth.bdname_len > 0 &&
       bufferLen > (index + dev->radio.bluetooth.bdname_len + PREAMBLE_LEN)) {
        ...
        if(dev->radio.bluetooth.bdname == NULL) {
            ...
            memcpy(
                dev->radio.bluetooth.bdname, buffer + index, dev->radio.bluetooth.bdname_len + 1);
        }
...
uint16_t parseBufferWifiAp(WendigoApp* app) {
   ...
   * packet terminator */
    uint8_t spurious =
        start_of_packet(buffer + PREAMBLE_LEN, bufferLen - PREAMBLE_LEN) + PREAMBLE_LEN;
    if(spurious < bufferLen - PREAMBLE_LEN) { // TODO THIS IS TRUE ALL THE TIME WTF
        wendigo_log_with_packet(MSG_ERROR, "Packet contains preamble", buffer, bufferLen);
        FURI_LOG_T(WENDIGO_TAG, "End parseBufferWifiAp()");
        return packetLen;
    }
    ...
    if(!memcmp(dev_mac, PREAMBLE_WIFI_AP, MAC_BYTES)) {
        wendigo_log_with_packet(MSG_ERROR, "AP with preamble in MAC", buffer, bufferLen);
        FURI_LOG_T(WENDIGO_TAG, "End parseBufferWifiAp()");
        return packetLen;
    }
    wendigo_device* dev = malloc(sizeof(wendigo_device));
...
uint16_t parseBufferWifiSta(WendigoApp* app) {
    ...
    if(spurious < bufferLen - PREAMBLE_LEN) {
        wendigo_log_with_packet(MSG_ERROR, "STA packet contains preamble", buffer, bufferLen);
        FURI_LOG_T(WENDIGO_TAG, "End parseBufferWifiSta()");
        return packetLen;
    }
    ...
    if(!memcmp(dev_mac, PREAMBLE_WIFI_STA, MAC_BYTES)) {
        wendigo_log_with_packet(
            MSG_ERROR, "STA packet with preamble in MAC skipper", buffer, bufferLen);
        FURI_LOG_T(WENDIGO_TAG, "End parseBufferWifiSta()");
        return packetLen;
    }
    wendigo_device* dev = malloc(sizeof(wendigo_device));
    ...
    if(memcmp(PACKET_TERM, buffer + WENDIGO_OFFSET_STA_TERM, PREAMBLE_LEN)) {
        ...
        wendigo_log_with_packet(
            MSG_ERROR,
            "STA packet terminator not found where expected, skipping.",
            buffer,
            packetLen);
    } else {
        if(memcmp(dev->radio.sta.apMac, nullMac, MAC_BYTES)) {
            wendigo_link_wifi_devices(
                app,
                dev,
                (uint8_t**)&dev->radio.sta.apMac,
    ...
    if(bufferLen > PREAMBLE_LEN && !memcmp(PREAMBLE_BT_BLE, buffer, PREAMBLE_LEN)) {
       ...
       throw away everything up to and including the end-of-packet sequence */
        ...
        if(consumedBytes == bufferLen) {
            ...
            wendigo_log_with_packet(
                MSG_ERROR, "Packet terminator not found in parseBuffer()", buffer, bufferLen);
            return;
        }
